...
/**
 * A embedded debug runtime with minimal debugger functionality.
 */
export class DebugRuntime extends EventEmitter {
	...
	// maps from sourceFile to array of debugger breakpoints
	private _breakPoints = new Map<string, IDebugBreakpoint[]>();
	...
	/**
	 * "Step into" for Embedded debug means: go to next character
	 */
	...
	/**
	 * "Step out" for embedded debug means: go to previous character
	 */
	public stepOut() {
		if (typeof this._currentColumn === 'number') {
			this._currentColumn -= 1;
	 ...
	 */
	public async setBreakPoint(path: string, line: number): Promise<IDebugBreakpoint> {

		const bp: IDebugBreakpoint = { verified: false, line, id: this._breakpointId++ };
		let bps = this._breakPoints.get(path);
		if (!bps) {
			bps = new Array<IDebugBreakpoint>();
	 ...
	 */
	public clearBreakPoint(path: string, line: number): IDebugBreakpoint | undefined {
