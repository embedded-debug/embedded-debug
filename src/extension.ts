...
export function activate(context: vscode.ExtensionContext) {
	...
	switch (runMode) {
		case 'server':
			...
			activateEmbeddedDebug(context, new MockDebugAdapterServerDescriptorFactory());
			break;

		case 'namedPipeServer':
			// run the debug adapter as a server inside the extension and communicate via a named pipe (Windows) or UNIX domain socket (non-Windows)
			activateEmbeddedDebug(context, new EmbeddedDebugAdapterNamedPipeServerDescriptorFactory());
			break;

		case 'external': default:
			// run the debug adapter as a separate process
			activateEmbeddedDebug(context, new DebugAdapterExecutableFactory());
			break;
	...
	createDebugAdapterDescriptor(session: vscode.DebugSession, executable: vscode.DebugAdapterExecutable | undefined): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
		...
		if (!this.server) {
			...
			this.server = Net.createServer(socket => {
				const session = new EmbeddedDebugSession(workspaceFileAccessor);
...
class EmbeddedDebugAdapterNamedPipeServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
