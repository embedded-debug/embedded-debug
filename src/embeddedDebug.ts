...
import { DebugRuntime, IDebugBreakpoint, FileAccessor } from './debugRuntime';
...
/**
 * This interface describes the embedded-debug specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the embedded-debug extension.
...
export class EmbeddedDebugSession extends LoggingDebugSession {
	...
	// a Embedded debug runtime (or debugger)
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', EmbeddedDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', EmbeddedDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', EmbeddedDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', EmbeddedDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, EmbeddedDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', EmbeddedDebugSession.threadID));
			}
		});
		this._runtime.on('breakpointValidated', (bp: IDebugBreakpoint) => {
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(EmbeddedDebugSession.threadID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'embedded-adapter-data');
	}
}
